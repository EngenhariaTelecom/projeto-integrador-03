...
void setup() {
  ...
  // Inicializa desligado (HIGH = relé desligado)
...
void loop() {
  ...
  if (mode == AUTO) {
    // Controle automático com histerese - desliga a descarga (HIGH)
  ...
  // Envia para o serial as seguintes informações
  Serial.print("Vbat: "); Serial.print(v_batt, 3);
  Serial.print(" V | Mode: "); Serial.print((mode == AUTO) ? "AUTO" : "MANUAL");
  Serial.print(" | Charge: "); Serial.print(forceCharge ? "ON" : "OFF");
  Serial.print(" | Disch: "); Serial.println(forceDischarge ? "ON" : "OFF");
